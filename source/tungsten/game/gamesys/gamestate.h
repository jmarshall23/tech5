...
// Original PDB header: w:\tech5\tungsten\game\gamesys\gamestate.h
// Recovered logical types: 6
// Signatures retain Xbox 360 ABI evidence and may still require manual review.


// IDA Local Type ordinal 1334; PDB kind: enum.
enum gameDLCState_t : __int32
{
  GAME_DLC_STATE_DEFAULT = 0x0,
  GAME_DLC_STATE_AUTHORITY = 0x1,
  GAME_DLC_STATE_SEWERS = 0x2,
};

// IDA Local Type ordinal 1532; PDB kind: enum.
enum gameState_t : __int32
{
  GAMESTATE_UNINITIALIZED = 0x0,
  GAMESTATE_NOMAP = 0x1,
  GAMESTATE_STARTUP = 0x2,
  GAMESTATE_ACTIVE = 0x3,
  GAMESTATE_SHUTDOWN = 0x4,
};

// IDA Local Type ordinal 1717; PDB kind: enum.
enum idGameStateInt::gameStateIntCompare_t : __int32
{
  GAMESTATEINT_LESS_THAN = 0x0,
  GAMESTATEINT_EQUAL = 0x1,
  GAMESTATEINT_GREATER_THAN = 0x2,
};

// IDA Local Type ordinal 14445; PDB kind: class.
class idGameStateInt : public idClass
{
public:
  // Recovered virtual interface; IDA vtable ordinal 14446.
  virtual idTypeInfo *GetType();
  virtual ~idGameStateInt();

  const idDeclGameStateInt *decl;
  int gameStateValue;
};

// IDA Local Type ordinal 18784; PDB kind: class.
class idGameState
{
public:
};

// IDA Local Type ordinal 19546; PDB kind: class.
class __declspec(align(8)) idProp_GameStateInt : public idProp_Moveable
{
public:
  // Recovered virtual interface; IDA vtable ordinal 19547.
  virtual idTypeInfo *GetType();
  virtual ~idProp_GameStateInt();
  ...
  virtual void InitPropPhysics();
  virtual void Pickup(idEntity *, const bool);
  virtual const idMaterial *GetIcon();
  virtual const char *GetIconName();
  virtual void Respawn();

  const idDeclGameStateInt *gameStateInt;
  int gameStateValue;
  float angle;
};
