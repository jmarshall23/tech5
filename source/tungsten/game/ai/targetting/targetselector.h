...
// Original PDB header: w:\tech5\tungsten\game\ai\targetting\targetselector.h
// Recovered logical types: 4
// Signatures retain Xbox 360 ABI evidence and may still require manual review.


// IDA Local Type ordinal 1068; PDB kind: unknown.
enum idItemSelector::<unnamed_tag> : __int32
{
  DISCARD = 0xFFFFFFFF,
};

// IDA Local Type ordinal 17312; PDB kind: class.
class idItemSelector : public idClass
{
public:
  // Recovered virtual interface; IDA vtable ordinal 17313.
  virtual idTypeInfo *GetType();
  virtual ~idItemSelector();
  virtual void InternalPreSelectItem();
  virtual float InternalGetWeight(const idInventoryItem *);
  virtual void InternalPostSelectItem();

  const idEntity *owner;
};

// IDA Local Type ordinal 18994; PDB kind: class.
class idTargetSelector
{
public:
  // Recovered virtual interface; IDA vtable ordinal 18995.
  virtual ~idTargetSelector();
  virtual float GetWeight(const idEntity *, const idTargetInfo *);

  idVec3 ownerDir;
};

// IDA Local Type ordinal 19419; PDB kind: class.
class __declspec(align(8)) idTarget_Teleport : public idEntity
{
public:
  // Recovered virtual interface; IDA vtable ordinal 19420.
  virtual idTypeInfo *GetType();
  virtual ~idTarget_Teleport();
