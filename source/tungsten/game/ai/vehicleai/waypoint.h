...
// Original PDB header: w:\tech5\tungsten\game\ai\vehicleai\waypoint.h
// Recovered logical types: 5
// Signatures retain Xbox 360 ABI evidence and may still require manual review.


// IDA Local Type ordinal 1038; PDB kind: enum.
enum aimPoint_t : __int32
{
  AIMPOINT_ORIGIN = 0x0,
  AIMPOINT_HEAD = 0x1,
  AIMPOINT_TORSO = 0x2,
  ...
  AIMPOINT_FEET = 0x5,
  AIMPOINT_BEST = 0x6,
  AIMPOINT_EYELEVEL = 0x7,
  AIMPOINT_LOOKAHEAD = 0x8,
  AIMPOINT_MAX = 0x9,
};

// IDA Local Type ordinal 15632; PDB kind: class.
class idWaypoint : public idEntity
{
public:
  // Recovered virtual interface; IDA vtable ordinal 15633.
...
// IDA Local Type ordinal 15710; PDB kind: class.
class idSplineWaypoint : public idWaypoint
{
public:
  // Recovered virtual interface; IDA vtable ordinal 15711.
  virtual idTypeInfo *GetType();
  virtual ~idSplineWaypoint();
  virtual idEventArg *CallEvent(idEventArg *result, const idEventDef *, const idEventArg *);
  virtual bool RespondsTo(const idEventDef *);
  virtual idEventArg *InternalCallEvent(idEventArg *result, const idEventDef *, const idEventArg *);
  virtual bool InternalRespondsTo(const idEventDef *);
  ...
  idNavSplinePosition splinePathPosition;
  float delay;
  const idDeclVoiceOver *voiceOver;
  vehicleSeat_t voSeat;
  const idDeclVoiceOver *voiceOver2;
  vehicleSeat_t voSeat2;
  const idSoundShader *sound;
  bool passThroughWaypoint;
  vaiControl_t control;
  vaiAction_t action;
  float speedLimit;
  float adaptToTargetSpeed;
  float radius;
  idList<idEntityPtr<idVehicleAI>,5> users;
};

// IDA Local Type ordinal 29935; PDB kind: typedef.
typedef st_lwPoint lwPoint;

// IDA Local Type ordinal 31949; PDB kind: typedef.
