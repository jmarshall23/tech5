...
// Original PDB header: w:\tech5\tungsten\game\entities\vehiclemountedturret.h
// Recovered logical types: 3
// Signatures retain Xbox 360 ABI evidence and may still require manual review.


// IDA Local Type ordinal 1654; PDB kind: enum.
enum idVehicleMountedTurret::weapon_t : __int32
{
  WEAPON_MACHINEGUN = 0x0,
  WEAPON_ROCKETLAUNCHER = 0x1,
  WEAPON_MAX = 0x2,
};

// IDA Local Type ordinal 1655; PDB kind: enum.
enum idVehicleMountedTurret::seatState_t : __int32
{
  SEATSTATE_LOWERED = 0x0,
  SEATSTATE_RAISED = 0x1,
  SEATSTATE_MAX = 0x2,
};

// IDA Local Type ordinal 18912; PDB kind: class.
class idVehicleMountedTurret : public idAnimatedEntity
{
public:
  // Recovered virtual interface; IDA vtable ordinal 18913.
  virtual idTypeInfo *GetType();
  virtual ~idVehicleMountedTurret();
  ...
  const idDeclEntityDef *gunnerDef;
  float slerpRate;
  idJointName yawJointName;
  idJointName pitchJointName;
  const idDeclWeapon *weaponDecl;
  idFaction faction;
  idStrRenderModel shieldModels[2];
  idStr shieldModelTags[2];
  idClipModel *shieldClipModels[2];
  float heatGainPerShot;
  float heatLostPerFrame;
  idAnimator_AnimWeb animWeb;
  idEntityPtr<idTurretGunner> gunner;
  idVec3 targetPos;
  idQuat lastYawRot;
  idQuat lastPitchRot;
  idIndex<short,enum invalidJointIndex_t> yawJoint;
  idIndex<short,enum invalidJointIndex_t> pitchJoint;
  idInventoryCollection inventory;
  idWeapon *weapon;
  idVehicleMountedTurret::weapon_t selectedWeapon;
  idVehicleMountedTurret::seatState_t seatState;
  bool hasValidTarget;
  bool safeFireMode;
