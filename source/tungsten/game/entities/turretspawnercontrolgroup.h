...
// Original PDB header: w:\tech5\tungsten\game\entities\turretspawnercontrolgroup.h
// Recovered logical types: 1
// Signatures retain Xbox 360 ABI evidence and may still require manual review.


// IDA Local Type ordinal 18973; PDB kind: class.
class __declspec(align(8)) idTurretSpawnerControlGroup : public idAIHintGroup
{
public:
  // Recovered virtual interface; IDA vtable ordinal 18974.
  virtual idTypeInfo *GetType();
  virtual ~idTurretSpawnerControlGroup();
  ...
  float gunnerStartingHitPoints;
  float bonusDamageWhenTaunting;
  float damageScaleFromTank;
  float visibleSightTime;
  float minTriggerDownDuration;
  float maxTriggerDownDuration;
  float minFireInterval;
  float maxFireInterval;
  float minPostTauntWaitToFire;
  float maxPostTauntWaitToFire;
  int maxLivingMinions;
  const idSoundShader *startTurretSound;
  const idSoundShader *endTurretSound;
