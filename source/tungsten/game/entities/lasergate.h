...
// Original PDB header: w:\tech5\tungsten\game\entities\lasergate.h
// Recovered logical types: 4
// Signatures retain Xbox 360 ABI evidence and may still require manual review.


// IDA Local Type ordinal 1887; PDB kind: enum.
enum laserGateState_t : __int32
{
  LASERGATE_OPENING = 0x0,
  LASERGATE_OPENED = 0x1,
  LASERGATE_CLOSING = 0x2,
  LASERGATE_CLOSED = 0x3,
};

// IDA Local Type ordinal 19801; PDB kind: class.
class idDeclLaserGate : public idDeclTypeInfo
{
public:
  // Recovered virtual interface; IDA vtable ordinal 19802.
  virtual ~idDeclLaserGate();
  ...
  int timeBetweenPoweringUpBeams;
  int beamPowerUpTime;
  const idSoundShader *activationSound;
  const idSoundShader *deactivationSound;
  const idSoundShader *gatePassThroughSound;
  const idDeclFX *fxDecl;
};

// IDA Local Type ordinal 19803; PDB kind: class.
class __declspec(align(2)) idLaserGate : public idEntity
{
public:
  // Recovered virtual interface; IDA vtable ordinal 19804.
...
// IDA Local Type ordinal 20314; PDB kind: class.
