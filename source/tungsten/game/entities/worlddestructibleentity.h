...
// Original PDB header: w:\tech5\tungsten\game\entities\worlddestructibleentity.h
// Recovered logical types: 2
// Signatures retain Xbox 360 ABI evidence and may still require manual review.


// IDA Local Type ordinal 18820; PDB kind: struct.
struct idWorldDestructibleEntity::destructiblePiece_t
{
  float health;
  int timeWhenBroken;
  const idDeclDestructiblePieceInfo::pieceInfo_t *pieceInfo;
};

// IDA Local Type ordinal 18831; PDB kind: class.
class __declspec(align(8)) idWorldDestructibleEntity : public idEntity
{
public:
  // Recovered virtual interface; IDA vtable ordinal 18832.
  virtual idTypeInfo *GetType();
  virtual ~idWorldDestructibleEntity();
  ...
  float health;
  float firstBreakHealthPercentage;
  float totalBreakHealthPercentage;
  float noClipHealthPercentage;
  bool canBeDamagedByPlayer;
  idList<idWorldDestructibleEntity::destructiblePiece_t,5> destructiblePieces;
  const idDeclDestructiblePieceInfo *destructiblePieceDef;
  float maxHealth;
  int numPiecesBroken;
  int numPiecesBrokenThisFrame;
  int numPiecesThatUseOverallHealth;
  float nextPieceBreakHealth;
  idList<idHandle<int,enum invalidDecalHandle_t,-1>,5> activeDecals;
  idPhysics_StaticMulti physicsObj;
  idClipModel *staticBaseClipModel;
