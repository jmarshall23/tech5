...
idFile_MTP::idFile_MTP()
    : uniqID(0), position(0), mode(MTP_FS_READ), length(0),
...
bool idFile_MTP::Open(const char* filename, const mtpFileMode_t openMode) {
    const char* const safeName = filename == nullptr ? "" : filename;
    name = safeName;
    ...
    std::lock_guard<std::mutex> lock(mtpMutex);
    if (!SendRequest(0, static_cast<unsigned int>(openMode), OP_OPEN,
    ...
    if (mode == MTP_FS_APPEND) position = length;
    return true;
}

int idFile_MTP::Read(void* buffer, const int len) {
    return ReadOfs(static_cast<std::int64_t>(position), buffer, len);
}

int idFile_MTP::ReadOfs(const std::int64_t offset, void* buffer, const int len) {
    if (buffer == nullptr || len <= 0 || offset < 0
            || (mode != MTP_FS_READ && mode != MTP_FS_READ_WRITE
                && mode != MTP_FS_READ_NO_BUFFERING)) return 0;
    std::lock_guard<std::mutex> lock(mtpMutex);
    ...
    for (int attempt = 0; attempt < 5; ++attempt) {
        ...
        while (total < len) {
            ...
            if (chunk == 0) break;
            if (chunk > static_cast<unsigned int>(len - total)
...
int idFile_MTP::Write(const void* buffer, const int len) {
    return WriteOfs(static_cast<std::int64_t>(position), buffer, len);
}

int idFile_MTP::WriteOfs(const std::int64_t offset, const void* buffer,
        const int len) {
    if (buffer == nullptr || len <= 0 || offset < 0
            || (mode != MTP_FS_WRITE && mode != MTP_FS_READ_WRITE
                && mode != MTP_FS_APPEND)) return 0;
    std::lock_guard<std::mutex> lock(mtpMutex);
    int total = 0;
    while (total < len) {
        const int chunk = std::min(mtpWriteSize, len - total);
        if (!SendRequest(static_cast<std::uint64_t>(offset) + total,
                static_cast<unsigned int>(chunk), OP_WRITE, name.c_str())
                || mtpTCP.WriteBlocking(static_cast<const unsigned char*>(buffer)
                    + total, chunk, mtpTimeout) != chunk) {
            mtpTCP.Close();
            break;
        }
        ...
        const int accepted = static_cast<int>(GetUInt32BE(countBytes));
...
int idFile_MTP::Length() const {
    return length > 0x7FFFFFFFu ? 0x7FFFFFFF : static_cast<int>(length);
}

int idFile_MTP::Tell() const {
    return position > 0x7FFFFFFFu ? 0x7FFFFFFF : static_cast<int>(position);
}

int idFile_MTP::Seek(const long offset, const fsOrigin_t origin) {
