...
#else
	unsigned int ret = 0xFFFFFFFFu;

	CRC32_UpdateChecksum( ret, versionInfo.netVersion, static_cast< int >( strlen( versionInfo.netVersion ) ) );
...
========================
*/
void idSessionLocal::QueuePacket( idSysQueue< idQueuePacket, &idQueuePacket::queueNode > & queue, int time, const lobbyAddress_t & to, const void * data, int size, bool dedicated ) {
	//mem.PushHeap();
...
leaderboardDefinition_t * Sys_CreateLeaderboardDef( int id_, int numColumns_, const columnDef_t * columnDefs_,
	...
	int leaderboardHandle = -1;
	for ( int index = 0; index < registeredLeaderboards.Num(); ++index ) {
		if ( registeredLeaderboards[ index ] == NULL ) { leaderboardHandle = index; break; }
	}
...
void Sys_DestroyLeaderboardDefs() {
	...
	for ( int index = 0; index < registeredLeaderboards.Num(); ++index ) {
		delete registeredLeaderboards[ index ];
	}
	registeredLeaderboards.Clear();
}
...
void idSessionLocal::ReadTitleStorage( void * buffer, int bufferLen ) {
	...
	idLexer parser( LEXFL_NOERRORS | LEXFL_NOFATALERRORS | LEXFL_NOSTRINGCONCAT );
	...
	while ( true ) {
		...
		if ( _stricmp( token.c_str(), "netvars" ) == 0 ) {
...
bool idSessionLocal::ReadDLCInfo( idDict & dlcInfo, void * buffer, int bufferLen ) {
	...
	while ( true ) {
		...
		if ( _stricmp( token.c_str(), "dlcInfo" ) == 0 ) {
