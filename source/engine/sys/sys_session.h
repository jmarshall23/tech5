...
public:
	...
	void Serialize( idSerializer & serializer ) {
		...
		int keyCount = serverInfo.GetNumKeyVals();
		serializer.SerializePacked( keyCount );
		...
		for ( int index = 0; index < keyCount; ++index ) {
			...
			if ( serializer.IsWriting() ) {
				const idKeyValue * keyValue = serverInfo.GetKeyVal( index );
				key = keyValue->GetKey();
			...
			if ( serializer.IsReading() ) {
				serverInfo.Set( key.c_str(), value.c_str() );
			}
		}
	}
...
public:
	...
	void WriteToMsg( idBitMsg & msg ) {
		...
		msg.WriteBits( lobbyType, 8 );
	}

	void ReadFromMsg( const idBitMsg & msg ) {
		localUserHandle.ReadFromMsg( msg );
		lobbyType = static_cast< byte >( msg.ReadBits( 8 ) );
	}
...
public:
	...
	// Debugging
	virtual void						DrawDebugNetworkHUD( idRenderModelGui * gui ) const = 0;
	virtual void						DrawDebugNetworkHUD2( idRenderModelGui * gui ) const = 0;
	virtual void						DrawDebugNetworkHUD_ServerSnapshotMetrics( idRenderModelGui * gui, bool draw ) = 0;
};
