...
// The Windows queue stores its link in a member node (the retail Xbox PDB
// records the equivalent byte offset as the template argument).  Keep the
// recovered two-pointer queue layout while making that distinction explicit.
template< typename type, idQueueNode< type > type::* nodeMember >
class idSysQueue {
public:
	idSysQueue() : first( NULL ), last( NULL ) {}
	bool IsEmpty() const { return first == NULL; }
	type * Peek() const { return first; }
	void Add( type * item ) {
		( item->*nodeMember ).next = NULL;
		if ( last != NULL ) ( last->*nodeMember ).next = item;
		else first = item;
		last = item;
	}
	type * RemoveFirst() {
...
public:
	...
	virtual void						DrawDebugNetworkHUD( idRenderModelGui * gui ) const {}
	virtual void						DrawDebugNetworkHUD2( idRenderModelGui * gui ) const {}
	virtual void						DrawDebugNetworkHUD_ServerSnapshotMetrics( idRenderModelGui * gui, bool draw ) {}
private:
...
public:
	...
	// Title Storage Vars
	//=====================================================================================================
	virtual float			GetTitleStorageFloat( const char * name, float defaultFloat ) const { float value = defaultFloat; titleStorageVars.GetFloat( name, defaultFloat, value ); return value; }
	virtual int				GetTitleStorageInt( const char * name, int defaultInt ) const { int value = defaultInt; titleStorageVars.GetInt( name, defaultInt, value ); return value; }
	virtual bool			GetTitleStorageBool( const char * name, bool defaultBool ) const { int value = defaultBool ? 1 : 0; titleStorageVars.GetInt( name, value, value ); return value != 0; }
	...
	virtual bool			GetTitleStorageBool( const char * name, bool defaultBool, bool & out ) const { int value = defaultBool ? 1 : 0; const bool found = titleStorageVars.GetInt( name, value, value ); out = value != 0; return found; }
	virtual bool			GetTitleStorageString( const char * name, const char * defaultString, const char ** out ) const { const idKeyValue * value = titleStorageVars.FindKey( name ); *out = value != NULL ? value->GetValue().c_str() : defaultString; return value != NULL; }
...
protected:
	...
	idSysQueue< idQueuePacket, &idQueuePacket::queueNode > sendQueue;
	idSysQueue< idQueuePacket, &idQueuePacket::queueNode > recvQueue;
	...
	void	QueuePacket( idSysQueue< idQueuePacket, &idQueuePacket::queueNode > & queue, int time, const lobbyAddress_t & to, const void * data, int size, bool dedicated );
