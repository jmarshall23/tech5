...
struct lobbyUser_t {
	...
	void WriteToMsg( idBitMsg & msg ) {
		...
		msg.WriteBits( peerIndex, 32 );
		msg.WriteBits( pingMs, -16 );
		msg.WriteBits( partyToken, 32 );
	...
	void ReadFromMsg( idBitMsg & msg ) {
		...
		peerIndex = msg.ReadBits( 32 );
		pingMs = msg.ReadBits( -16 );
		partyToken = static_cast< uint32 >( msg.ReadBits( 32 ) );
	...
	void WriteClientMutableData( idBitMsg & msg ) {
		...
		msg.WriteBits( weaponAutoSwitch ? 1 : 0, 1 );
		msg.WriteBits( weaponAutoReload ? 1 : 0, 1 );
		assert( msg.writeBit == 0 );
	}
	...
	void ReadClientMutableData( idBitMsg & msg ) {
		...
		weaponAutoSwitch = msg.ReadBits( 1 ) != 0;
		weaponAutoReload = msg.ReadBits( 1 ) != 0;
	}
};
