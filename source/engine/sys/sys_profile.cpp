...
void idProfileMgr::OnLoadSettingsCompleted( idSaveLoadParms * parms ) {
	...
	idFile_SaveGame * profileFile = NULL;
	for ( int index = 0; index < parms->files.Num(); ++index ) {
		if ( _stricmp( parms->files[ index ]->GetName(), SAVEGAME_PROFILE_FILENAME ) == 0 ) {
			profileFile = parms->files[ index ];
			break;
		}
	}
	...
	if ( foundProfile ) {
		...
		NetworkReadBig( profileFile, originalChecksum );

		int dataLength = profileFile->Length() - (int)sizeof( originalChecksum );
		profileFile->Read( buffer.Ptr(), dataLength );
...
bool idSaveGameProcessorSaveProfile::InitSaveProfile( idPlayerProfile * profile_, const char * folder ) {
	...
	NetworkWriteBig( profileFile, checksum );
...
bool Sys_SaveGameProfileCheck() {
	...
	if ( fileSystem->IsFolder( saveFolder, FSPATH_SAVE ) == FOLDER_YES ) {
		...
		for ( int i = 0; i < files->GetNumFiles(); i++ ) {
			if ( _stricmp( files->GetFile( i ), SAVEGAME_PROFILE_FILENAME ) == 0 ) {
				exists = true;
				break;
			}
		}
