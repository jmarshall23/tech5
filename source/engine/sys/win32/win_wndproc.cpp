	...
	switch( uMsg ) {
		case WM_WINDOWPOSCHANGED:
			{
				...
				if ( ::GetClientRect( hWnd, &rect ) && rect.right > rect.left && rect.bottom > rect.top ) {
					...
					if ( ( style & WS_POPUP ) == 0 ) {
						r_windowWidth.SetInteger( rect.right - rect.left );
						r_windowHeight.SetInteger( rect.bottom - rect.top );
					}
				}
			}
			break;
		...
		case WM_SYSKEYDOWN:
			if ( wParam == 13 ) {	// alt-enter toggles full-screen
				cvarSystem->SetCVarBool( "r_fullscreen", !cvarSystem->GetCVarBool( "r_fullscreen", false ), 0 );
				cmdSystem->AppendCommandText( "vid_restart\n" );
				return 0;
			}
...
//			win32.movingWindow = true;
		...
		case WM_MOUSEMOVE: {
			...
			const bool isShellActive = ( game != NULL && game->Shell_IsActive() );
			const bool isConsoleActive = false;
