...
static bool Net_ExtractPort( const char *src, char *buf, int bufsize, int *port ) {
	...
	p = buf; p += Min( bufsize - 1, static_cast< int >( strlen( src ) ) ); *p = '\0';
	p = strchr( buf, ':' );
	if ( !p ) {
		return false;
	}
	...
	if( ( dwRetVal = GetAdaptersInfo( pAdapterInfo, &ulOutBufLen) ) != NO_ERROR ) {
		...
		while( pAdapter ) {
			...
			while( pIPAddrString ) {
				...
				strncpy_s( netint[num_interfaces].addr, sizeof( netint[num_interfaces].addr ),
					pIPAddrString->IpAddress.String, _TRUNCATE );
				num_interfaces++;
...
const char *Sys_NetAdrToString( const netadr_t a ) {
	...
	if ( a.type == NA_LOOPBACK ) {
		if ( a.port ) {
			_snprintf_s( s, 64, _TRUNCATE, "localhost:%i", a.port );
		} else {
			_snprintf_s( s, 64, _TRUNCATE, "localhost" );
		}
	} else if ( a.type == NA_IP ) {
		_snprintf_s( s, 64, _TRUNCATE, "%i.%i.%i.%i:%i", a.ip[0], a.ip[1], a.ip[2], a.ip[3], a.port );
	}
	return s;
}
...
========================
*/
bool idUDP::InitForPort( int portNumber, bool useBackend ) {
	(void)useBackend;
