...
void GLW_CheckWGLExtensions( HDC hDC ) {
	...
	r_swapInterval.flags |= CVAR_MODIFIED;	// force a set next frame
...
bool R_GetModeListForDisplay( const int requestedDisplayNum, idList<vidMode_t> & modeList ) {
	...
	for ( int displayNum = requestedDisplayNum; ; displayNum++ ) {
		...
		if ( modeList.Num() > 0 ) {
			...
			std::sort( modeList.Ptr(), modeList.Ptr() + modeList.Num(),
				[]( const vidMode_t & a, const vidMode_t & b ) {
					const int hd = a.height - b.height;
					const int wd = a.width - b.width;
					return hd != 0 ? hd < 0 : ( wd != 0 ? wd < 0 : a.displayHz < b.displayHz );
				} );
...
bool GLimp_Init( glimpParms_t parms ) {
	...
	cmdSystem->AddCommand( "testSwapBuffers", GLimp_TestSwapBuffers, "Times swapbuffer options" );
...
void GLimp_SwapBuffers() {
	if ( ( r_swapInterval.flags & CVAR_MODIFIED ) != 0 ) {
		r_swapInterval.flags &= ~CVAR_MODIFIED;
