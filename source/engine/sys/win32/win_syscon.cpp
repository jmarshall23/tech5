...
// framework/EditField is outside this recovery's authorized system tree.  The
// native console uses only its fixed edit buffer and cursor-facing methods, so
...
public:
	...
	void SetCursor( int value ) { cursor = (std::max)( 0, (std::min)( value, MAX_EDIT_LINE - 1 ) ); }
	void Clear() { buffer[ 0 ] = '\0'; cursor = 0; }
	void AutoComplete() { }
...
static char * WinConsoleCopyString( const char * text ) {
	const char * source = text != NULL ? text : "";
	const size_t length = strlen( source ) + 1;
	char * copy = static_cast< char * >( Mem_Alloc( length, TAG_SYSTEM ) );
	if ( copy != NULL ) {
...
static bool WinConsoleIsColor( const char * text ) {
	return text != NULL && text[ 0 ] == '^' && text[ 1 ] >= '0' && text[ 1 ] <= '9';
}
		...
		case WM_COMMAND:
			if ( wParam == COPY_ID ) {
				...
				if ( s_wcd.quitOnClose ) {
					...
					cmdString = WinConsoleCopyString( "quit" );
...
#define CONSOLE_BUFFER_SIZE		16384
	...
	while ( msg[i] && ( ( b - buffer ) < sizeof( buffer ) - 1 ) ) {
		if ( msg[i] == '\n' && msg[i+1] == '\r' ) {
			...
			b[1] = '\n';
			b += 2;
		} else if ( WinConsoleIsColor( &msg[i] ) ) {
			i++;
		} else {
			*b= msg[i];
			b++;
...
void Win_SetErrorText( const char *buf ) {
	strncpy_s( s_wcd.errorString, sizeof( s_wcd.errorString ),
		buf != NULL ? buf : "", _TRUNCATE );
	if ( !s_wcd.hwndErrorBox ) {
