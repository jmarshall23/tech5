...
const char * Sys_GetOSUserName() {
	return Sys_GetCurrentUser();
}

const char * Sys_GetMachineName() {
	static char machineName[MAX_COMPUTERNAME_LENGTH + 1] = {};
	if ( machineName[0] == '\0' ) {
		DWORD length = static_cast<DWORD>( sizeof( machineName ) );
		if ( !GetComputerNameA( machineName, &length ) || machineName[0] == '\0' ) {
			strcpy_s( machineName, sizeof( machineName ), "localhost" );
		}
	}
	return machineName;
}
