...
static const int MAX_PRINT_MSG = 4096;
const char * Sys_EXEPath();

static void WinMainStripTrailing( idStr & text, const char * suffix ) {
	const int suffixLength = static_cast< int >( strlen( suffix ) );
	if ( suffixLength <= 0 || text.Length() < suffixLength ||
		strcmp( text.c_str() + text.Length() - suffixLength, suffix ) != 0 ) {
		return;
	}
	idStr trimmed;
	for ( int index = 0; index < text.Length() - suffixLength; ++index ) {
		trimmed.Append( text.c_str()[ index ] );
	}
	text = trimmed;
}
...
void Sys_Printf( const char *fmt, ... ) {
	...
	va_list argptr;
	va_start(argptr, fmt);
	_vsnprintf_s( msg, MAXPRINTMSG, _TRUNCATE, fmt, argptr );
	va_end(argptr);
	msg[sizeof(msg)-1] = '\0';
...
void Sys_DebugVPrintf( const char *fmt, va_list arg ) {
	...
	_vsnprintf_s( msg, MAXPRINTMSG, _TRUNCATE, fmt, arg );
	msg[ sizeof(msg)-1 ] = '\0';
...
int Sys_ListFiles( const char *directory, const char *extension, idList< idStr > &list ) {
	...
	search.Format( "%s\\*%s", directory, extension );
	...
	if ( OpenClipboard( NULL ) != 0 ) {
		...
		if ( ( hClipboardData = GetClipboardData( CF_TEXT ) ) != 0 ) {
			if ( ( cliptext = (char *)GlobalLock( hClipboardData ) ) != 0 ) {
				data = (char *)Mem_Alloc( GlobalSize( hClipboardData ) + 1, TAG_TEMP );
...
bool Sys_Exec(	const char * appPath, const char * workingPath, const char * args, 
	...
	unsigned int & exitCode ) {
		...
		char * cmdLine = NULL;
		{
			...
			if ( args != NULL ) {
				if ( appPath != NULL ) {
					int len = static_cast< int >( strlen( args ) + strlen( appPath ) ) + 1 /* for space */ + 1 /* for NULL terminator */ + 2 /* app quotes */;
					...
					_snprintf_s( cmdLine, len, _TRUNCATE, "\"%s\" %s", appPath, args );
				} else {
					int len = static_cast< int >( strlen( args ) ) + 1;
					cmdLine = (char*)Mem_Alloc( len, TAG_TEMP );
					strncpy_s( cmdLine, len, args, _TRUNCATE );
				}
...
=====================
*/
uint32 Sys_DLL_Load( const char *dllName ) {
	HINSTANCE libHandle = LoadLibrary( dllName );
	return static_cast<uint32>( reinterpret_cast<uintptr_t>( libHandle ) );
}
...
=====================
*/
void *Sys_DLL_GetProcAddress( uint32 dllHandle, const char *procName ) {
	return reinterpret_cast<void *>( GetProcAddress( reinterpret_cast<HINSTANCE>( static_cast<uintptr_t>( dllHandle ) ), procName ) );
}
...
=====================
*/
void Sys_DLL_Unload( uint32 dllHandle ) {
	if ( !dllHandle ) {
		return;
	}
	if ( FreeLibrary( reinterpret_cast<HINSTANCE>( static_cast<uintptr_t>( dllHandle ) ) ) == 0 ) {
...
void Sys_QueEvent( sysEventType_t type, int value, int value2, int ptrLength, void *ptr, int inputDeviceNum ) {
	sysEvent_t * ev = &eventQue[ eventHead % MAX_QUED_EVENTS ];
...
sysEvent_t Sys_GetEvent() {
	...
	if ( eventHead > eventTail ) {
		eventTail++;
		return eventQue[ ( eventTail - 1 ) % MAX_QUED_EVENTS ];
	}
...
//	SetTimer( NULL, 0, 100, NULL );
	...
	cmdSystem->AddCommand( "in_restart", Sys_In_Restart_f, "restarts the input system", NULL );
	...
	if ( !idStr::Icmp( win32.sys_cpustring.GetString(), "detect" ) ) {
		...
		if ( win32.cpuid & CPUID_AMD ) {
			string.Append( "AMD CPU" );
		} else if ( win32.cpuid & CPUID_INTEL ) {
			string.Append( "Intel CPU" );
		} else if ( win32.cpuid & CPUID_UNSUPPORTED ) {
			string.Append( "unsupported CPU" );
		} else {
			string.Append( "generic CPU" );
		}
		...
		WinMainStripTrailing( string, " & " );
		WinMainStripTrailing( string, " with " );
		win32.sys_cpustring.SetString( string );
	} else {
		idLib::Printf( "forcing CPU type to " );
		idLexer src;
		src.LoadMemory( win32.sys_cpustring.GetString(),
			static_cast< unsigned int >( strlen( win32.sys_cpustring.GetString() ) ),
			"sys_cpustring" );
		idToken token;

		int id = CPUID_NONE;
		while( src.ReadToken( token ) ) {
			if ( idStr::Icmp( token.c_str(), "generic" ) == 0 ) {
				id |= CPUID_GENERIC;
			} else if ( idStr::Icmp( token.c_str(), "intel" ) == 0 ) {
				id |= CPUID_INTEL;
			} else if ( idStr::Icmp( token.c_str(), "amd" ) == 0 ) {
				id |= CPUID_AMD;
			} else if ( idStr::Icmp( token.c_str(), "mmx" ) == 0 ) {
				id |= CPUID_MMX;
			} else if ( idStr::Icmp( token.c_str(), "3dnow" ) == 0 ) {
				id |= CPUID_3DNOW;
			} else if ( idStr::Icmp( token.c_str(), "sse" ) == 0 ) {
				id |= CPUID_SSE;
			} else if ( idStr::Icmp( token.c_str(), "sse2" ) == 0 ) {
				id |= CPUID_SSE2;
			} else if ( idStr::Icmp( token.c_str(), "sse3" ) == 0 ) {
				id |= CPUID_SSE3;
			} else if ( idStr::Icmp( token.c_str(), "htt" ) == 0 ) {
				id |= CPUID_HTT;
			}
		}
		if ( id == CPUID_NONE ) {
...
void Win_Frame() {
	...
	if ( ( win32.win_viewlog.flags & CVAR_MODIFIED ) != 0 ) {
		win32.win_viewlog.flags &= ~CVAR_MODIFIED;
	}
}
...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow ) {
	...
	strncpy_s( sys_cmdline, sizeof( sys_cmdline ), lpCmdLine != NULL ? lpCmdLine : "", _TRUNCATE );
...
//	Sys_FPU_EnableExceptions( TEST_FPU_EXCEPTIONS );
	...
	idLib::Printf( "%s", Sys_FPU_GetState() );
#endif
...
void idSysLocal::OpenURL( const char *url, bool doexit ) {
	...
	if (doexit_spamguard) {
		idLib::Debugf( "OpenURL: already in an exit sequence, ignoring %s\n", url );
		return;
	}
