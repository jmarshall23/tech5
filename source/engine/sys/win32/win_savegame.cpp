...
void SavegameAppendPath( idStr & path, const char * component ) {
	if ( path.Length() > 0 ) {
		const char tail = path.c_str()[ path.Length() - 1 ];
		if ( tail != '/' && tail != '\\' ) {
			path.Append( "/" );
		}
	}
...
void SavegameStripFilename( idStr & path ) {
	const char * text = path.c_str();
	int slash = -1;
	for ( int index = 0; text[ index ] != '\0'; ++index ) {
		if ( text[ index ] == '/' || text[ index ] == '\\' ) slash = index;
	}
	if ( slash < 0 ) {
		path.Clear();
	} else {
		idStr parent;
		for ( int index = 0; index < slash; ++index ) parent.Append( text[ index ] );
		path = parent;
	}
}

void SavegameStripPath( idStr & path ) {
	const char * text = path.c_str();
...
int SavegameIcmpPrefix( const idStr & text, const idStr & prefix ) {
	return _strnicmp( text.c_str(), prefix.c_str(), prefix.Length() );
}

idStr SavegameRight( const idStr & text, int count ) {
	count = idMath::ClampInt( 0, text.Length(), count );
	return idStr( text.c_str() + text.Length() - count );
}
...
void Sys_ExecuteSavegameCommandAsyncImpl( idSaveLoadParms * savegameParms ) {
	...
	if ( !session->GetSaveGameManager().GetSaveGameThread().IsRunning() ) {
...
idLocalUserWin * GetLocalUserFromSaveParms( const saveGameThreadArgs_t & data ) {
	if ( ( data.saveLoadParms != NULL) && ( data.saveLoadParms->inputDeviceId >= 0 ) ) {
		...
		if ( user != NULL ) {
			...
			if ( userWin != NULL && data.saveLoadParms->userId == static_cast< int >( Sys_StringHash( userWin->GetGamerTag() ) ) ) {
				return userWin;
			}
		}
	}
...
int idSaveGameThread::Save() {
	...
	SavegameAppendPath( saveFolder, callback->directory );
	...
	idList< idStr > filesToDelete;
	if ( ( callback->mode & SAVEGAME_MBF_DELETE_FILES ) && !callback->cancelled ) {
		if ( fileSystem->IsFolder( saveFolder.c_str(), FSPATH_SAVE ) == FOLDER_YES ) {
	...
	if ( requiredSizeBytes > 0 && !callback->cancelled ) {
		...
		if ( callback->requiredSpaceInBytes > 0 ) {
			...
			directory.Append( "\\" );	// so it doesn't think the last part is a file and ignores in the directory creation
	...
	for ( int i = 0; i < filesToDelete.Num() && !callback->cancelled; i++ ) {
		fileSystem->RemoveFile( filesToDelete[i].c_str(), FSPATH_SAVE );
	}
	...
	for ( int i = 0; i < callback->files.Num() && ret == ERROR_SUCCESS && !callback->cancelled; i++ ) {
		...
		SavegameAppendPath( fileName, file->GetName() );
		idStr tempFileName = va( "%s.temp", fileName.c_str() );

		idFile * outputFile = fileSystem->OpenFileWrite( tempFileName, FSPATH_SAVE );
		if ( outputFile == NULL ) {
		...
		if ( ( file->type & SAVEGAMEFILE_PIPELINED ) != 0 ) {
			...
			if ( ( file->type & SAVEGAMEFILE_BINARY ) || ( file->type & SAVEGAMEFILE_COMPRESSED ) ) {
				if ( saveGame_checksum.GetBool() ) {
					...
					size_t size = NetworkWriteBig( outputFile, checksum );
		...
		if ( ret == ERROR_SUCCESS ) {
			// Remove the old file
			if ( !fileSystem->RenameFile( tempFileName, fileName, FSPATH_SAVE ) ) {
				idLib::Warning( "Could not start to rename temporary file %s to %s.", tempFileName.c_str(), fileName.c_str() );
			}
		} else {
			fileSystem->RemoveFile( tempFileName, FSPATH_SAVE );
...
int idSaveGameThread::Load() {
	...
	if ( fileSystem->IsFolder( saveFolder, FSPATH_SAVE ) != FOLDER_YES ) {
			...
			if ( ( file->type & SAVEGAMEFILE_BINARY ) != 0 || ( file->type & SAVEGAMEFILE_COMPRESSED ) != 0 ) {
				if ( saveGame_checksum.GetBool() ) {
					if ( size >= sizeof( originalChecksum ) ) {
						NetworkReadBig( inputFile, originalChecksum );
...
int idSaveGameThread::Delete() {
	...
	if ( fileSystem->IsFolder( saveFolder, FSPATH_SAVE ) == FOLDER_YES ) {
		...
		for ( int i = 0; i < files->GetNumFiles() && !callback->cancelled; i++ ) {
			fileSystem->RemoveFile( files->GetFile( i ), FSPATH_SAVE );
		}
		fileSystem->FreeFileList( files );
...
int idSaveGameThread::Enumerate() {
	...
	if ( fileSystem->IsFolder( saveFolder, FSPATH_SAVE ) == FOLDER_YES ) {
		...
		const idList< idStr > & fileList = files->list;
		...
		for ( int i = 0; i < fileList.Num() && !callback->cancelled; i++ ) {
			...
			SavegameStripFilename( directory );
...
int idSaveGameThread::EnumerateFiles() {
	...
	SavegameAppendPath( folder, callback->directory );

	callback->files.Clear();

	int ret = ERROR_SUCCESS;
	if ( fileSystem->IsFolder( folder, FSPATH_SAVE ) == FOLDER_YES ) {
		...
		for ( int i = 0; i < files->GetNumFiles() && ret == 0 && !callback->cancelled; i++ ) {
			...
			SavegameStripPath( filename );

			if ( SavegameIcmpPrefix( filename, callback->pattern ) != 0 ) {
				continue;
			}
			if ( callback->postPattern.Length() != 0 &&
				SavegameIcmpPrefix( SavegameRight( filename, callback->postPattern.Length() ), callback->postPattern ) != 0 ) {
				continue;
			}
			...
			if ( callback->mode & SAVEGAME_MBF_READ_DETAILS ) {
				...
				assert( fileSystem->IsFolder( details.slotName, FSPATH_SAVE ) == FOLDER_YES );
			}
...
int idSaveGameThread::DeleteFiles() {
	...
	for ( int j = 0; j < callback->files.Num() && !callback->cancelled; ++j ) {
		...
		SavegameAppendPath( fullpath, file->GetName() );
		fileSystem->RemoveFile( fullpath, FSPATH_SAVE );
	}
	...
	if ( fileSystem->IsFolder( folder, FSPATH_SAVE ) == FOLDER_YES ) {
		...
		for ( int i = 0; i < files->GetNumFiles() && !callback->cancelled; i++ ) {
			...
			if ( callback->pattern.Length() == 0 || SavegameIcmpPrefix( filename, callback->pattern ) != 0 ) {
				continue;
			}
			if ( callback->postPattern.Length() == 0 ||
				SavegameIcmpPrefix( SavegameRight( filename, callback->postPattern.Length() ), callback->postPattern ) != 0 ) {
...
int idSaveGameThread::DeleteAll() {
	...
	if ( fileSystem->IsFolder( saveFolder, FSPATH_SAVE ) == FOLDER_YES ) {
		...
		for ( int i = 0; i < files->GetNumFiles() && !callback->cancelled; i++ ) {
			...
			if ( fileSystem->IsFolder( files->GetFile( i ), FSPATH_SAVE ) == FOLDER_YES ) {
...
void Sys_SaveGameCheck( bool & exists, bool & autosaveExists ) {
	...
	if ( fileSystem->IsFolder( saveFolder, FSPATH_SAVE ) == FOLDER_YES ) {
		...
		for ( int i = 0; i < fileList.Num(); i++ ) {
			...
			if ( fileSystem->IsFolder( directory, FSPATH_SAVE ) == FOLDER_YES ) {
				exists = true;
