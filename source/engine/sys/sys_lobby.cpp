...
void idLobby::HandlePacket( lobbyAddress_t & remoteAddress, idBitMsg fragMsg, idPacketProcessor::sessionId_t sessionID ) {
	...
	if ( type == idPacketProcessor::RETURN_TYPE_OOB ) {	
		...
		for ( int r = 0; r < numReliable; r++ ) {
			...
			NetworkMsgSetSize( reliableMsg, reliableSize );
		...
		if ( msg.GetRemainingData() > 0 && loaded ) {
			...
			if ( peerNum == host ) {
				...
				common->NetReceiveUsercmds( usercmdMsg );
			}
		}
	}
}
...
void idLobby::HandleReliablePlayerToPlayerMsg( int peerNum, idBitMsg & msg, int type ) {
	...
	int c, b;
	c = msg.readCount;
	b = msg.readBit; // in case we need to forward or fail
...
uint32 idLobby::GetPartyTokenAsHost() {
	...
	if ( partyToken == 0 ) {
		...
		if ( masterUser != NULL ) {
			seed += Sys_StringHash( masterUser->GetGamerTag() );
		}
...
void idLobby::DrawDebugNetworkHUD( idRenderModelGui * gui ) const {
	if ( gui == NULL ) {
		return;
	}
	int		totalSendRate = 0;
	...
	const idVec4 green( 0.0f, 1.0f, 0.0f, 1.0f );
	const idVec4 red( 1.0f, 0.0f, 0.0f, 1.0f );
	gui->DrawFilled( idVec4( 0.0f, 0.0f, 0.0f, 0.7f ), X_OFFSET - 10.0f, curY - 10.0f, 1550, ( peers.Num() + numLines ) * Y_SPACING + 20.0f );

	gui->DrawString( X_OFFSET, curY, "# Peer                   | Sent kB/s | Recv kB/s | Sent MB | Recv MB | Ping   | L |  %  | R.NM | R.SZ | R.AK | T", green, false, 1.0f );
	curY += Y_SPACING;

	gui->DrawString( X_OFFSET, curY, "------------------------------------------------------------------------------------------------------------------------------------", green, false, 1.0f );
	curY += Y_SPACING;
	...
	for ( int p = 0; p < peers.Num(); p++ ) {		
		...
		const idVec4 & color = sentKps > 20.0f ? red : green;

		int resourcePercent = 0;
		idStr name = peer.address.ToString();
		
		name.Append( lobbyType == TYPE_PARTY ? "(P": "(G" );
		name.Append( host == p ? ":H)" : ":C)" );

		gui->DrawString( X_OFFSET, curY, va( "%i %22s | %2.02f kB/s | %2.02f kB/s | %2.02f MB | %2.02f MB |%4i ms | %i | %i%% | %i | %i | %i | %2.2f / %2.2f / %i", p, name.c_str(), sentKps, recvKps, sentMB, recvMB, peer.lastPingRtt, peer.loaded, resourcePercent, peer.packetProc->NumQueuedReliables(), peer.packetProc->GetReliableDataSize(), peer.packetProc->NeedToSendReliableAck(), peer.snapHz, peer.maxSnapBps, peer.failedPingRecoveries ), color, false, 1.0f );
		curY += Y_SPACING;
	}
	...
	const idVec4 & color = totalSentKps > 100.0f ? red : green;
	
	gui->DrawString( X_OFFSET, curY, va( "# %20s | %2.02f KB/s | %2.02f KB/s | %2.02f MB | %2.02f MB", "", totalSentKps, totalRecvKps, totalSentMB, totalRecvMB ), color, false, 1.0f );
	curY += Y_SPACING;
	...
	if ( net_forceUpstream.GetFloat() != 0.0f ) {
		...
		gui->DrawString( X_OFFSET, curY, va( "Queued: %d | Dropping: %2.02f kB/s Queue: %2.02f kB/s -> Effective %2.02f kB/s", queuedBytes, upstreamDropRate / 1024.0f, upstreamQueuedRate / 1024.0f, totalSentKps - ( upstreamDropRate / 1024.0f ) + ( upstreamQueuedRate / 1024.0f ) ), color, false, 1.0f );
	}
}
...
========================
*/
void idLobby::DrawDebugNetworkHUD2( idRenderModelGui * gui ) const {
	if ( gui == NULL ) {
		return;
	}
	int		totalSendRate = 0;
	...
	const idVec4 green( 0.0f, 1.0f, 0.0f, 1.0f );
	const idVec4 red( 1.0f, 0.0f, 0.0f, 1.0f );
	gui->DrawFilled( idVec4( 0.0f, 0.0f, 0.0f, 0.7f ), X_OFFSET - 10.0f, curY - 10.0f, 550, (peers.Num() + 4) * Y_SPACING + 20.0f );	

	const char* stateName = session->GetStateString();

	gui->DrawFilled( idVec4( 1.0f, 1.0f, 1.0f, 0.7f ), X_OFFSET - 10.0f, curY - 10.0f, 550, (peers.Num() + 5) * Y_SPACING + 20.0f );

	gui->DrawString( X_OFFSET, curY, va("State: %s. Local time: %d", stateName, Sys_Milliseconds() ), green, false, 1.0f );
	curY += Y_SPACING;

	gui->DrawString( X_OFFSET, curY, "Peer           | Sent kB/s | Recv kB/s | L | R | Resources", green, false, 1.0f );
	curY += Y_SPACING;
	...
	for ( int p = 0; p < peers.Num(); p++ ) {
		...
		const idVec4 & color = proc.TickFragmentAccumulator() ? red : green;
		...
		char peerName[ 11 ];
		if ( IsHost() ) {
			_snprintf_s( peerName, sizeof( peerName ), _TRUNCATE, "%-10.10s", GetPeerName( p ) );
		} else {
			strcpy_s( peerName, "Local     " );
		}

		gui->DrawString( X_OFFSET, curY, va( "%i - %s | %2.02f kB/s | %2.02f kB/s | %i | %i | %d/%d", p, peerName, sentKps, recvKps, peers[p].loaded, peers[p].address.UsingRelay(), rLoaded, rTotal ), color, false, 1.0f );
		curY += Y_SPACING;
	}
	...
	gui->DrawString( X_OFFSET, curY, va( "Total | %2.02f KB/s | %2.02f KB/s", totalSentKps, totalRecvKps ), green, false, 1.0f );
}
...
void idLobby::DrawDebugNetworkHUD_ServerSnapshotMetrics( idRenderModelGui * gui, bool draw ) {
	if ( gui == NULL || !draw ) {
		return;
	}
	const float Y_OFFSET	= 20.0f;
	const float X_OFFSET	= 20.0f;
	const float Y_SPACING	= 15.0f;
	const idVec4 color( 1.0f, 1.0f, 1.0f, 1.0f );
	...
	for ( int p = 0; p < peers.Num(); p++ ) {
		...
		gui->DrawFilled( idVec4( 0.0f, 0.0f, 0.0f, 0.7f ), X_OFFSET - 10.0f, curY - 10.0f, width, ( Y_SPACING * numLines ) + 20.0f );

		gui->DrawString( X_OFFSET, curY, va( "Peer %d - %s RTT %d %sPeerSnapRate: %d %s", p, GetPeerName( p ), peer.lastPingRtt, throttled ? "^1" : "^2", peer.throttledSnapRate/1000, throttled ? "^1Throttled" : ""  ), color, false, 1.0f );
		curY += Y_SPACING;

		gui->DrawString( X_OFFSET, curY, va( "SnapSeq %d  BaseSeq %d  Delta %d  Queue %d", snapSeq, snapBase, deltaSeq, snapProc->GetSnapQueueSize() ), color, false, 1.0f );
		curY += Y_SPACING;

		gui->DrawString( X_OFFSET, curY, va( "Reliables: %d / %d bytes Reliable Ack: %d", packetProc->NumQueuedReliables(), packetProc->GetReliableDataSize(), packetProc->NeedToSendReliableAck() ), color, false, 1.0f );
		curY += Y_SPACING;

		gui->DrawString( X_OFFSET, curY, va( "Outgoing %.2f kB/s  Reported %.2f kB/s Throttle: %.2f", peer.packetProc->GetOutgoingRateBytes() / 1024.0f, peers[p].receivedBps / 1024.0f, peer.receivedThrottle ), color, false, 1.0f );
		curY += Y_SPACING;
		...
		if ( net_forceUpstream.GetFloat() != 0.0f ) {
			...
			float upstreamQueuedRate = session->GetUpstreamQueueRate();
			int queuedBytes = session->GetQueuedBytes();
			gui->DrawString( X_OFFSET, curY, va( "Queued: %d | Dropping: %2.02f kB/s Queue: %2.02f kB/s ", queuedBytes, upstreamDropRate / 1024.0f, upstreamQueuedRate / 1024.0f ), color, false, 1.0f );
