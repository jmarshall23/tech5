...
void idSessionLocal::OnSaveCompleted( idSaveLoadParms * parms ) {
	...
	if ( common->Game() != NULL ) common->Game()->OnSaveCompleted( parms );
}
...
saveGameHandle_t idSessionLocal::LoadGameSync( const char * name, saveFileEntryList_t & files ) {
...

	{
		...
		// Done this way so we know it will be shutdown properly on early exit or exception
		struct local_t {
			local_t( idSaveLoadParms * parms_, const char * slot_ ) : parms( parms_ ), slot( slot_ ) {
			...
			~local_t() { 
				...
				if ( common->Game() != NULL ) common->Game()->OnLoadCompleted( parms, &slot );
			}
			idSaveLoadParms * parms;
			idStr slot;
		} local( &parms, name );
		...
		for ( int i = 0; i < details.Num(); ++i ) {
			if ( idStr::Cmp( name, details[i].slotName ) == 0 ) {
				if ( common->Game() != NULL && details[i].GetSaveVersion() > common->Game()->GetGameVersion() ) {
...
void idSessionLocal::OnEnumerationCompleted( idSaveLoadParms * parms ) {
	...
	if ( common->Game() != NULL ) common->Game()->OnEnumerationCompleted( parms );
}
...
void idSessionLocal::OnDeleteCompleted( idSaveLoadParms * parms ) {
	...
	if ( common->Game() != NULL ) common->Game()->OnDeleteCompleted( parms );
}
...
bool idSessionLocal::LoadGameCheckDescriptionFile( idSaveLoadParms & parms ) {
	idFile_SaveGame * detailsFile = NULL;
	for ( int index = 0; index < parms.files.Num(); ++index ) {
		if ( _stricmp( parms.files[ index ]->GetName(), SAVEGAME_DETAILS_FILENAME ) == 0 ) {
			detailsFile = parms.files[ index ];
			break;
		}
	}
	if ( detailsFile == NULL ) {
	...
	if ( !SavegameReadDetailsFromFile( detailsFile, parms.description ) ) {
		...
		if ( common->Game() != NULL && parms.description.GetSaveVersion() > common->Game()->GetGameVersion() ) {
			parms.errorCode = SAVEGAME_E_INCOMPATIBLE_NEWER_VERSION;
		}
	}
