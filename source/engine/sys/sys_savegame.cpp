...
idStr RemoveSaveFolderPrefix( const char * folder, idSaveGameManager::packageType_t type ) {
	...
	if ( prefixLength > 0 && strncmp( dir.c_str(), prefix.c_str(), prefixLength ) == 0 ) {
		dir = dir.c_str() + prefixLength;
	}
	return dir;
}
...
bool SavegameReadDetailsFromFile( idFile * file, idSaveGameDetails & details ) {
	...
	int ignoreChecksumValue = 0;
	details.descriptors.GetInt( "ignore_checksum", 0, ignoreChecksumValue );
	const bool ignoreChecksum = ignoreChecksumValue != 0;
	if ( !ignoreChecksum ) {
		// Get the checksum from the dict
		int readChecksum = 0;
		details.descriptors.GetInt( SAVEGAME_DETAIL_FIELD_CHECKSUM, 0, readChecksum );
...
void idSaveLoadParms::SetDefaults( int newInputDevice ) {
	...
	if ( user != NULL ) {
		...
		userId = Sys_StringHash( userWin->GetGamerTag() );
...
void idSaveLoadParms::CancelSaveGameFilePipelines() {
	for ( int i = 0; i < files.Num(); i++ ) {
		if ( ( files[i]->type & SAVEGAMEFILE_PIPELINED ) != 0 ) {
			...
			if ( file->mode == idFile_SaveGamePipelined::WRITE ) {
			...
			} else if ( file->mode == idFile_SaveGamePipelined::READ ) {
...
bool idSaveGameManager::DeviceSelectorWaitingOnSaveRetry() {
	...
	return ( _stricmp( retryFolder, "GAME-autosave" ) == 0 );
}
...
void idSaveGameManager::ShowRetySaveDialog( const char * folder, const int64 bytes ) {
	...
	public:
		idSWFScriptVar * Call( idSWFScriptVar * result, idSWFScriptObject * thisObject, const idSWFParmList * parms ) override {
			common->Dialog().ClearDialog( GDM_INSUFFICENT_STORAGE_SPACE );
			session->GetSaveGameManager().ClearRetryInfo();
			*result = idSWFScriptVar();
			return result;
		}
	};
...
void idSaveGameManager::Pump() {
	...
	if ( processor->IsThreadFinished() ) {
		...
		for ( int i = 0; i < localProcessor->completedCallbacks.Num(); ++i ) {
			delete localProcessor->completedCallbacks[i];
		}
		localProcessor->completedCallbacks.Clear();
