...
public:
	...
	void SubmitPendingSnap( int visIndex, uint8 * objMemory, int objMemorySize,
		lzwCompressionData_t * lzwData, idParallelJobList * snapshotJobList = NULL );
	// GetPendingSnapDelta
	...
	int GetLastAppendedSequence() { return deltas.items.Num() == 0 ? -1 : deltas.items[ deltas.items.Num() - 1 ].sequence; }

	int	GetSnapQueueSize() { return deltas.items.Num(); }
...
private:
	...
	idArray< idSerializerLog *, MAX_SNAPSHOT_QUEUE > deltaLogs;
	...
	static_assert( sizeof( jobMemory_t ) == 214080,
		"Recovered snapshot job memory ABI changed" );
	...
	void SetDeltaSerializerLog( int sequence, idSerializerLog * log );
};

#if defined( _WIN32 ) && !defined( _WIN64 )
static_assert( sizeof( idSnapshotProcessor ) == 66864,
	"Recovered idSnapshotProcessor ABI changed" );
#endif
