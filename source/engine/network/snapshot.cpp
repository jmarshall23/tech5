...
idSnapShot::idSnapShot( const idSnapShot & other ) :
	serializeLog( NULL ), time( 0 ), recvTime( 0 ), isGlobalHeap( mem.IsGlobalHeap() ) {
	*this = other;
}
...
void idSnapShot::Clear() {
	...
	SetSerializerLog( NULL );
}

void idSnapShot::SetSerializerLog( idSerializerLog * log ) {
	if ( log != NULL ) {
		log->AddRef();
	}
	if ( serializeLog != NULL ) {
		serializeLog->Release();
	}
	serializeLog = log;
}
...
void idSnapShot::operator=( const idSnapShot & other ) {
	...
	if ( this != &other ) {
		const int previousNum = objectStates.Num();
		for ( int i = other.objectStates.Num(); i < objectStates.Num(); i++ ) {
			FreeObjectState( i );
		}
		objectStates.SetNum( other.objectStates.Num() );
		for ( int i = previousNum; i < objectStates.Num(); ++i ) {
			objectStates[ i ] = NULL;
		}
		for ( int i = 0; i < objectStates.Num(); i++ ) {
		...
		SetSerializerLog( other.serializeLog );
	}
}
...
void idSnapShot::SubmitLZWJob( 
	...
	if ( serializeLog != NULL ) {
		for ( int index = 0; index < numObjects; ++index ) {
			serializeLog->AddCompressionLog( writeDeltaInfo.headers[ index ].objID,
				writeDeltaInfo.headers[ index ].csize );
		}
	}

	// Until the recovered Windows idParallelJobList methods are present, use
	// the same authoritative job body synchronously.
	(void)writeDeltaInfo.snapshotJobList;
	LZWJob( curlzwParm );
...
bool idSnapShot::ReadDelta( idFile * file, int visIndex ) {
	...
	NetworkReadBig( file, time );

	int objectNum = 0;
	uint16 delta = 0;
	while ( NetworkReadBig( file, delta ) == sizeof( delta ) ) {	
		objectNum += delta;
		if ( objectNum >= 0xFFFF ) {
			// full delta
			return true;
		...
		if ( newsize == 0 ) {
			...
			for ( objectSize_t i = 0; i < compareSize; i++ ) {
				uint8 delta = 0;
				NetworkReadBig< byte >( file, delta );
...
void idSnapShot::WriteObject( idFile * file, int visIndex, objectState_t * newState, objectState_t * oldState, int & lastobjectNum ) {
	...
	if ( newState == NULL ) {
		...
		if ( visChange ) {
			...
			NetworkWriteBig< objectSize_t >( file, visSendState ? SIZE_NOT_STALE : SIZE_STALE );
		}
		if ( !visChange || visSendState ) {
			...
			NetworkWriteBig( file, newState->buffer.Size() );										// Write new size
			
			// Compare bytes that overlap
			for ( objectSize_t b = 0; b < compareSize; b++ ) {
				NetworkWriteBig< byte >( file, static_cast< byte >( ( 0xFF + 1 + newState->buffer[b] - oldState->buffer[b] ) & 0xFF ) );
			}
...
int idSnapShot::GetObjectMsgByIndex( int i, idBitMsg & msg, bool ignoreIfStale ) const {
	...
	msg.Init( static_cast< const byte * >( state.buffer.Ptr() ), state.buffer.Size() );
