...
*/
idSnapshotProcessor::~idSnapshotProcessor() {
	for ( int index = 0; index < deltaLogs.Num(); ++index ) {
		if ( deltaLogs[ index ] != NULL ) {
			deltaLogs[ index ]->Release();
			deltaLogs[ index ] = NULL;
		}
	}
	//mem.PushHeap();
	Mem_Free( jobMemory );
...
void idSnapshotProcessor::Reset( bool cstor ) {
	...
	pendingSnap.Clear();
	NetworkQueueClear( deltas );
	memset( deltas.data, 0, sizeof( deltas.data ) );
	if ( cstor ) {
		memset( &deltaLogs, 0, sizeof( deltaLogs ) );
	} else {
		for ( int index = 0; index < deltaLogs.Num(); ++index ) {
			if ( deltaLogs[ index ] != NULL ) {
				deltaLogs[ index ]->Release();
...
void idSnapshotProcessor::SetDeltaSerializerLog( int sequence, idSerializerLog * log ) {
	idSerializerLog *& slot = deltaLogs[ sequence % MAX_SNAPSHOT_QUEUE ];
	if ( slot != NULL ) {
		slot->Release();
		slot = NULL;
	}
...
========================
*/
void idSnapshotProcessor::SubmitPendingSnap( int visIndex, uint8 * objMemory, int objMemorySize,
		lzwCompressionData_t * lzwData, idParallelJobList * snapshotJobList ) {
	...
	submitInfo.snapshotJobList	= snapshotJobList;
...
int idSnapshotProcessor::GetPendingSnapDelta( byte * outBuffer, int maxLength ) {
	...
	if ( deltaBaseSequence >= baseSequence ) {	
		if ( !NetworkQueueAppend( deltas, snapSequence, deltaData, size ) ) {
			int resendLength = NetworkQueueLength( deltas, NetworkQueueNum( deltas ) - 1 );

			if ( !verify( resendLength <= maxLength ) ) {
				idLib::Error( "GetPendingSnapDelta: Size overflow for resend." );
			}

			memcpy( outBuffer, NetworkQueueData( deltas, NetworkQueueNum( deltas ) - 1 ), resendLength );
			 size = -resendLength;
		}
		SetDeltaSerializerLog( snapSequence, pendingSnap.GetSerializerLog() );
	}
	...
	idSerializerLog * pendingLog = pendingSnap.GetSerializerLog();
	if ( pendingLog != NULL && !pendingLog->fragmented ) {
		pendingLog->fragmented = !jobMemory->lzwInOutData.fullSnap;
		pendingLog->finalCompressedSize = size;
	}
...
bool idSnapshotProcessor::ReceiveSnapshotDelta( const byte * deltaData, int deltaLength, int visIndex, int & outSeq, int & outBaseSeq, idSnapShot & outSnap, bool & fullSnap ) {
	...
	// Save the new delta
	if ( net_skipClientDeltaAppend.GetBool() || !NetworkQueueAppend( deltas, deltaSequence, deltaData, deltaLength ) ) {
	...
	if ( NetworkQueueNum( deltas ) > 10 ) {
		NET_VERBOSESNAPSHOT_PRINT("NET: ReceiveSnapshotDelta: deltas.Num() > 10: %d\n   ", NetworkQueueNum( deltas ) );
		for ( int i=0; i < NetworkQueueNum( deltas ); i++ ) {
			NET_VERBOSESNAPSHOT_PRINT( "%d ", NetworkQueueSequence( deltas, i ) );
		}
	...
	SetDeltaSerializerLog( deltaSequence, outSnap.GetSerializerLog() );

	// Make a copy of the basestate the server used to create this delta, and then apply and return it
	outSnap = baseState;
	outSnap.SetSerializerLog( deltaLogs[ deltaSequence % MAX_SNAPSHOT_QUEUE ] );
	
	fullSnap = ApplyDeltaToSnapshot( outSnap, (const char *)deltaData, deltaLength, visIndex );
	idSerializerLog * receivedLog = outSnap.GetSerializerLog();
	if ( receivedLog != NULL && !receivedLog->fragmented ) {
		receivedLog->fragmented = !fullSnap;
		receivedLog->finalCompressedSize = deltaLength;
	}
...
bool idSnapshotProcessor::ApplySnapshotDelta( int visIndex, int snapshotNumber ) {
	...
	if ( NetworkQueueNum( deltas ) == 0 || NetworkQueueSequence( deltas, 0 ) != snapshotNumber ) {
	...
	PeekDeltaSequence( (const char *)NetworkQueueData( deltas, 0 ), NetworkQueueLength( deltas, 0 ), deltaSequence, deltaBaseSequence );
	...
	if ( ApplyDeltaToSnapshot( baseState, (const char *)NetworkQueueData( deltas, 0 ), NetworkQueueLength( deltas, 0 ), visIndex ) ) {
		lastFullSnapBaseSequence = deltaSequence;
	}
	
	baseSequence = deltaSequence;		// This is now our new base sequence
	baseState.SetSerializerLog( deltaLogs[ deltaSequence % MAX_SNAPSHOT_QUEUE ] );
...
void idSnapshotProcessor::RemoveDeltasForOldBaseSequence() {
	...
	for ( int i = NetworkQueueNum( deltas ) - 1; i >= 0; i-- ) {
		int deltaSequence		= 0;
		int deltaBaseSequence	= 0;
		baseState.PeekDeltaSequence( (const char *)NetworkQueueData( deltas, i ), NetworkQueueLength( deltas, i ), deltaSequence, deltaBaseSequence );
		if ( deltaBaseSequence < baseSequence ) {
			// Remove this delta, and all deltas before this one 
			deltas.RemoveOlderThan( NetworkQueueSequence( deltas, i ) + 1 );
			break;
		}
	}
}
...
void idSnapshotProcessor::SanityCheckDeltas() {
	...
	for ( int i = 0; i < NetworkQueueNum( deltas ); i++ ) {
		baseState.PeekDeltaSequence( (const char *)NetworkQueueData( deltas, i ), NetworkQueueLength( deltas, i ), deltaSequence, deltaBaseSequence );
		assert( deltaSequence == NetworkQueueSequence( deltas, i ) );	// Make sure delta stored in compressed form matches the one stored in the data queue
