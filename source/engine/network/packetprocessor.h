...
public:
	...
	idPacketProcessor()
		: outgoingPacketSize( idStr( "outgoingPacketSize" ), idStr( "size of outgoing packets" ) ),
		  incomingPacketSize( idStr( "incomingPacketSize" ), idStr( "size of incoming packets" ) ),
		  handledReliableMsgSize( idStr( "handledReliableSize" ), idStr( "size of handled reliable msgs" ) ),
		  queuedReliableMsgSize( idStr( "queuedReliableSize" ), idStr( "size of queued reliable msgs" ) ),
		  fragmentedPacketSize( idStr( "fragmentedPacketSize" ), idStr( "size of fragmented packets" ) ),
		  sentSnapShotSize( idStr( "sentSnapShotSize" ), idStr( "size of sent snapshots" ) ) {
		Reset();
	}
	...
	// RAGE PDB type information fixes this capacity at 63.  The BFG branch
	// raised it to 64 after the code lines diverged.
	static const int MAX_RELIABLE_QUEUE		= 63;
...
public:		
	...
	int NumQueuedReliables() { return reliable.items.Num(); }
	...
	int				GetReliableDataSize() const		{ return reliable.dataLength; }
...
private:
	...
	public:
		...
		void WriteToMsg( idBitMsg & msg ) {
			msg.WriteBits( sessionID, 16 );	
		}

		void ReadFromMsg( idBitMsg & msg ) {
			sessionID = static_cast< sessionId_t >( msg.ReadBits( 16 ) );	
		}
	...
	// Present in the RAGE PDB and absent from the later BFG branch.
...
static_assert( sizeof( idPacketProcessor ) == 34032, "Recovered idPacketProcessor ABI changed" );
#endif
