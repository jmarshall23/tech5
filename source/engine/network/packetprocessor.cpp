...
int idPacketProcessor::FinalizeRead( idBitMsg & inMsg, idBitMsg & outMsg, int & userValue ) {
	...
	if ( header.Type() == PACKET_TYPE_RELIABLE_ACK ) {
		// Handle reliable ack
		int reliableSequence = inMsg.ReadBits( 32 );
	...
	if ( header.Type() == PACKET_TYPE_OOB ) {
		...
		if ( numReliableRecv > 0 ) {
			// Byte align msg
			NetworkMsgReadByteAlign( inMsg );

			int compressedSize = inMsg.ReadBits( 16 );
			...
			NetworkMsgSetReadCount( inMsg, inMsg.GetReadCount() + compressedSize );
	...
	NetworkMsgSetSize( outMsg, inMsg.GetRemainingData() );
...
void idPacketProcessor::UpdateOutgoingRate( const int time, const int size ) {
	...
	if ( size != 0 ) {
		outgoingPacketSize.Log( static_cast< float >( size ), time != 0 );
	}
}
...
void idPacketProcessor::UpdateIncomingRate( const int time, const int size ) {
	...
	if ( size != 0 ) {
		incomingPacketSize.Log( static_cast< float >( size ), time != 0 );
	}
}
...
bool idPacketProcessor::ProcessOutgoing( const int time, const idBitMsg & msg, bool isOOB, int userData ) {
	...
	if ( queuedReliableAck >= 0 ) {
		...
		unsentMsg.WriteBits( queuedReliableAck, 32 );
		queuedReliableAck = -1;
	}
	...
	if ( isOOB ) {
		...
		idInnerPacketHeader header( PACKET_TYPE_INBAND, NetworkQueueNum( reliable ) );
		header.WriteToMsg( unsentMsg );
		if ( NetworkQueueNum( reliable ) > 0 ) {
			// Byte align unsentMsg 
			NetworkMsgWriteByteAlign( unsentMsg );
			
			lzwCompressionData_t	lzwData;
			idLZWCompressor			lzwCompressor( &lzwData );

			lzwCompressor.Start( unsentMsg.GetWriteData() + unsentMsg.GetSize() + 2, unsentMsg.GetMaxSize() - unsentMsg.GetSize() - 2 );		// Write to compressed mem, not exceeding MAX_MSG_SIZE (+2 to reserve space for compressed size)

			int uncompressedSize = 4;
			lzwCompressor.WriteAgnostic< int >( NetworkQueueSequence( reliable, 0 ) );
			for ( int i = 0; i < NetworkQueueNum( reliable ); i++ ) {
				const uint16 reliableLength = static_cast< uint16 >( NetworkQueueLength( reliable, i ) );
				lzwCompressor.WriteAgnostic< uint16 >( reliableLength );
				lzwCompressor.Write( NetworkQueueData( reliable, i ), reliableLength );
				uncompressedSize += 2 + reliableLength;
			}
			...
			unsentMsg.WriteBits( lzwCompressor.Length(), 16 );
			NetworkMsgSetSize( unsentMsg, unsentMsg.GetSize() + lzwCompressor.Length() );
...
bool idPacketProcessor::GetSendFragment( const int time, sessionId_t sessionID, idBitMsg & outMsg ) {
	...
	if ( !fragmentedSend ) {
		...
		outMsg.WriteBits( fragmentSequence, 32 );
...
int idPacketProcessor::ProcessIncoming( int time, sessionId_t expectedSessionID, idBitMsg & msg, idBitMsg & out, int & userData, const int peerNum ) {
	...
	if ( header.Type() != PACKET_TYPE_FRAGMENTED ) {
		// Non fragmented
		msg.readCount = c; msg.readBit = b;		// Reset since we took a byte to check the type
		return FinalizeRead( msg, out, userData );
	} 

	// Decode fragmented packet
	int readSequence = msg.ReadBits( 32 );	// Read sequence of fragment
	...
	if ( header.Value() == FRAGMENT_END ) {
		...
		NetworkMsgSetSize( msg, msgWritePos );
		return FinalizeRead( msg, out, userData );
	}
...
bool idPacketProcessor::ProcessConnectionlessIncoming( idBitMsg & msg, idBitMsg & out, int & userData ) {
	...
	NetworkMsgSetSize( out, msg.GetRemainingData() );
...
void idPacketProcessor::VerifyEmptyReliableQueue( byte keepMsgBelowThis, byte replaceWithThisMsg ) {
	...
	if ( net_verifyReliableQueue.GetInteger() == 1 ) {
		idLib::Error( "reliable queue is not empty: %d messages", NetworkQueueNum( reliable ) );
		return;
	}
	idLib::Warning( "reliable queue is not empty: %d messages", NetworkQueueNum( reliable ) );
	...
	RELIABLE_VERBOSE( "rollback send sequence from %d to %d\n", reliableSequenceSend, NetworkQueueSequence( reliable, 0 ) );
	for ( int i = 0; i < NetworkQueueNum( reliable ); i++ ) {
		byte peek = NetworkQueueData( reliable, i )[0];
		if ( peek < keepMsgBelowThis ) {
			RELIABLE_VERBOSE( "keeping %d\n", peek );
			NetworkQueueAppend( clean, NetworkQueueSequence( reliable, i ), NetworkQueueData( reliable, i ), NetworkQueueLength( reliable, i ) );
		} else {
			...
			NetworkQueueAppend( clean, NetworkQueueSequence( reliable, i ), &replaceWithThisMsg, 1 );
			RELIABLE_VERBOSE( "dropping %d\n", peek );
		}
	}

	assert( NetworkQueueNum( reliable ) == NetworkQueueNum( clean ) );
