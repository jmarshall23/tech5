...
#undef HANDLE_SWF_TAG
		...
		case Tag_FrameLabel: {
				...
				label.frameLabel.Set( tagStream.ReadString() );
			}
			break;
...
void idSWFSprite::Read( idFile * f ) {
	...
	for ( int i = 0; i < frameLabels.Num(); i++ ) {
		...
		idStr frameLabel;
		f->ReadString( frameLabel );
		frameLabels[i].frameLabel.Set( frameLabel.c_str() );
	}
...
void idSWFSprite::Write( idFile * f ) {
	...
	for ( int i = 0; i < frameLabels.Num(); i++ ) {
		...
		f->WriteString( frameLabels[i].frameLabel.c_str() );
	}
