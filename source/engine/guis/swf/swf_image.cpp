...
// The JPEG declarations came from the original framework PCH.  Keep the
// implementation isolated here until the renderer's jpeg-6 dependency is
// recovered into the PC tree.

idCVar swf_imageCompressor( "swf_imageCompressor", "1", CVAR_INTEGER, "1 = extremely slow high quality compressor" );
...
void idSWF::WriteSwfImageAtlas( const char *filename ) {
	...
	atlasWidth = Max( 4, totalSize.x * 4 );
	atlasHeight = Max( 4, totalSize.y * 4 );
...
//			minV[0], maxV[0], minV[1], maxV[1], minV[2], maxV[2], minV[3], maxV[3] );
	...
	R_WriteTGA( filename, swfAtlas.Ptr(), atlasWidth, atlasHeight, 4, false, true, FSPATH_BASE );

	idStr binaryName( filename );
	binaryName.SetFileExtension( ".bimage" );
	...
	imageOpts.linear = false;
	imageOpts.cubeFilter = false;
	imageOpts.overlayMemory = false;
	imageOpts.startPurged = false;
	imageOpts.format = FMT_DXT5;
	imageOpts.colorFormat = swf_imageCompressor.GetInteger() != 0 ? CFM_YCOCGA_HQ_DXT5 : CFM_YCOCGA_DXT5;
	imageOpts.filter = TF_LINEAR;
	imageOpts.repeat = TR_CLAMP;
	...
	idImage::WriteBinaryImage( binaryName.c_str(), imageOpts, swfAtlas.Ptr(), 0 );
}
...
========================
*/
void idSWF::LoadImageA( int characterID, const byte * imageData, int width, int height ) {
...
void idSWF::DefineBits( idSWFBitStream & bitstream ) {
	...
	LoadImageA( characterID, imageData, width, height );
