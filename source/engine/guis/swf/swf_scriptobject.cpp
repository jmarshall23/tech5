...
========================
*/
idSWFScriptObject::idSWFScriptObject() : refCount( 1 ), prototype( NULL ), objectType( SWF_OBJECT_OBJECT ) {
	data.sprite = NULL;
	Clear();
}
...
========================
*/
idSWFScriptObject * idSWFScriptObject::GetScriptObject( const char * name ) {
	idSWFScriptVar var = Get( name );
	return var.IsObject() ? var.GetObject() : NULL;
}
...
void idSWFScriptObject::Set( const char * name, const idSWFScriptVar & value ) {
	if ( objectType == SWF_OBJECT_ARRAY ) {
		if ( idStr::Cmp( name, "length" ) == 0 ) {
			...
			for ( int i = 0; i < variables.Num(); i++ ) {
				int hash = 0;
				for ( const char * p = variables[i].name.c_str(); *p != '\0'; ++p ) {
					hash = 31 * hash + static_cast< unsigned char >( *p );
				}
				hash &= VARIABLE_HASH_BUCKETS - 1;
...
idSWFScriptVar idSWFScriptObject::DefaultValue( bool stringHint ) {
	...
	case SWF_OBJECT_SPRITE:
		if ( data.sprite != NULL ) {
			return idSWFScriptVar( va( "[sprite:%s]", data.sprite->GetName() ) );
		} else {
			return idSWFScriptVar( "[null sprite]" );
		}
...
idSWFScriptObject::swfNamedVar_t * idSWFScriptObject::GetVariable( int index, bool create ) {
	...
	if ( create ) {
		...
		char indexName[32];
		std::snprintf( indexName, sizeof( indexName ), "%d", index );
		variable->name.Set( indexName );
		variable->native = NULL;
		int hash = 0;
		for ( const char * p = variable->name.c_str(); *p != '\0'; ++p ) {
			hash = 31 * hash + static_cast< unsigned char >( *p );
		}
		hash &= VARIABLE_HASH_BUCKETS - 1;
...
void idSWFScriptObject::PrintToConsole() const {
	if ( objectType == SWF_OBJECT_SPRITE && data.sprite != NULL ) {
		idLib::Printf( "Sprite '%s' on frame %d (%s)\n", data.sprite->name.c_str(),
			data.sprite->currentFrame, data.sprite->isPlaying ? "playing" : "stopped" );
	}
	if ( objectType == SWF_OBJECT_TEXT && data.text != NULL ) {
		idLib::Printf( "Text '%s': %s\n", data.text->name.c_str(), data.text->text.c_str() );
	}
	if ( variables.Num() > 0 ) {
		idLib::Printf( "%d children:\n", variables.Num() );
		int maxVarLength = 0;

		for ( int i = 0; i < variables.Num(); ++i ) {
			const idSWFScriptObject::swfNamedVar_t & nv = variables[ i ];
			const int nameLength = static_cast< int >( std::strlen( nv.name.c_str() ) );
