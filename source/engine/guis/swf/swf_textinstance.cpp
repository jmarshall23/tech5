...
// Font services were supplied by the original framework PCH.
...
idSWFTextInstance::~idSWFTextInstance() {
	...
	if ( swf != NULL ) {
		const int orphanIndex = swf->FindTextOrphanIndex( this );
		if ( orphanIndex != -1 ) {
			swf->orphanedTexts.RemoveIndex( orphanIndex );
		}
	}
...
========================
*/
void idSWFTextInstance::Init( idSWFEditText * _editText, idSWFSpriteInstance * _parent, const char * _name, int _depth, idSWF * _swf ) {
	editText = _editText;
	swf = _swf;
	parent = _parent;
	depth = _depth;
...
/*
========================
idSWFTextInstance::ReInit
========================
*/
void idSWFTextInstance::ReInit() {
	assert( editText != NULL );
	assert( swf != NULL );

	if ( sys_langPolish.GetBool()
		&& idStr::Icmp( swf->GetName(), "swf/merchants.swf" ) == 0
		&& idStr::Icmp( editText->initialText.c_str(), "#str_swf_off" ) == 0 ) {
		text = "RABATU";
	} else {
		text = idLocalization::GetString( editText->initialText.c_str() );
	}

	lengthCalculated = false;
	variable = editText->variable.c_str();
...
idSWFScriptObject_TextInstancePrototype::idSWFScriptObject_TextInstancePrototype() {
	...
	SWF_TEXT_NATIVE_VAR_SET( tooltipBindset );
...
SWF_TEXT_NATIVE_VAR_DEFINE_SET( tooltipBindset ) { SWF_TEXT_PTHIS_SET( "tooltipBindset" ); pThis->bindset = value.ToInteger(); }
SWF_TEXT_NATIVE_VAR_DEFINE_GET( tooltipBindset ) { SWF_TEXT_PTHIS_GET( "tooltipBindset" ); return pThis->bindset; }
