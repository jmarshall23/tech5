...
bool idSWF::LoadSWF( const char * fullpath ) {
	...
	fileSystem->FixLongFilename( "generated", "tga", fullpath, atlasFileName, sizeof( atlasFileName ) );
...
===================
*/
bool idSWF::LoadBinary( const char * bfilename ) {
	...
	if (  magic != BSWF_MAGIC || ( !fileSystem->InProductionMode() && timestamp != btimestamp ) )  {
		delete f;
		return false;
	}
	...
	f->ReadBig( frameRate );
	f->ReadBig( atlasWidth );
	f->ReadBig( atlasHeight );
	...
	for ( int i = 0; i < dictionary.Num(); i++ ) {
		...
		switch ( dictionary[i].type ) {
			...
			case SWF_DICT_EDITTEXT: {
				...
				idStr variable;
				idStr initialText;
				f->ReadString( variable );
				f->ReadString( initialText );
				edittext->variable.Set( variable.c_str() );
				edittext->initialText.Set( initialText.c_str() );
				break;
			}
		}
	}
...
void idSWF::WriteBinary( const char * bfilename ) {
	...
	file->WriteBig( atlasWidth );
	file->WriteBig( atlasHeight );
	...
	for ( int i = 0; i < dictionary.Num(); i++ ) {
		...
		switch ( dictionary[i].type ) {
			...
			case SWF_DICT_EDITTEXT: {
				...
				file->WriteString( edittext->variable.c_str() );
				file->WriteString( edittext->initialText.c_str() );
				break;
			}
		}
	}
