...
========================
*/
void idSWFSpriteInstance::Init( idSWFSprite * _sprite, idSWFSpriteInstance * _parent, const char * _name, int _depth )  {
	sprite = _sprite;
	parent = _parent;
	depth = _depth;
	name.Set( _name );
	if ( parent != NULL ) {
		parent->scriptObject->AddRef();
	}
...
idSWFSpriteInstance::~idSWFSpriteInstance() {
	if ( parent != NULL ) {
		parent->scriptObject->Set( name.c_str(), idSWFScriptVar() );
		parent->scriptObject->Release();
	}
	for ( int i = 0; i < displayList.Num(); i++ ) {
		const int spriteOrphan = sprite->swf->FindSpriteOrphanIndex( displayList[i].spriteInstance );
		if ( spriteOrphan != -1 ) {
			sprite->swf->orphanedSprites.RemoveIndex( spriteOrphan );
		}
		const int textOrphan = sprite->swf->FindTextOrphanIndex( displayList[i].textInstance );
		if ( textOrphan != -1 ) {
			sprite->swf->orphanedTexts.RemoveIndex( textOrphan );
		}
...
void idSWFSpriteInstance::FreeDisplayList() {
	for ( int i = 0; i < displayList.Num(); i++ ) {
		if ( displayList[i].spriteInstance != NULL ) {
			sprite->swf->AddSpriteOrphan( displayList[i].spriteInstance );
		}
		if ( displayList[i].textInstance != NULL ) {
			sprite->swf->AddTextOrphan( displayList[i].textInstance );
		}
	}
...
========================
*/
swfDisplayEntry_t * idSWFSpriteInstance::AddDisplayEntry( int depth, int characterID, const idStr & objectName ) {
	int i = 0;
	...
	if ( dictEntry != NULL ) {
		if ( dictEntry->type == SWF_DICT_SPRITE ) {
			display.spriteInstance = sprite->swf->ExtractSpriteFromOrphanage( objectName, dictEntry->sprite, depth, this );
			if ( display.spriteInstance == NULL ) {
				display.spriteInstance = sprite->swf->spriteInstanceAllocator.Alloc();
				display.spriteInstance->Init( dictEntry->sprite, this, objectName.c_str(), depth );
				display.spriteInstance->RunTo( 1 );
			}
			scriptObject->Set( objectName.c_str(), idSWFScriptVar( display.spriteInstance->scriptObject ) );
		} else if ( dictEntry->type == SWF_DICT_EDITTEXT ) {
			display.textInstance = sprite->swf->ExtractTextFromOrphanage( objectName, dictEntry->edittext, depth, this );
			if ( display.textInstance == NULL ) {
				display.textInstance = sprite->swf->textInstanceAllocator.Alloc();
				display.textInstance->Init( dictEntry->edittext, this, objectName.c_str(), depth, sprite->GetSWF() );
			}
			scriptObject->Set( objectName.c_str(), idSWFScriptVar( display.textInstance->scriptObject ) );
		}
	}
	return &display;
}
...
void idSWFSpriteInstance::RemoveDisplayEntry( int depth ) {
	...
	if ( entry != NULL ) {
		if ( entry->spriteInstance != NULL ) {
			sprite->swf->AddSpriteOrphan( entry->spriteInstance );
		}
		if ( entry->textInstance != NULL ) {
			sprite->swf->AddTextOrphan( entry->textInstance );
		}
...
void idSWFSpriteInstance::SwapDepths( int depth1, int depth2 ) {
	for ( int i = 0; i < displayList.Num(); i++ ) {
		...
		if ( displayList[i].spriteInstance != NULL ) {
			...
			if ( displayList[i].spriteInstance->name.c_str()[0] == '\0' ) {
				idLib::Warning( "SWF: SwapDepths called on unnamed sprite. Please give this sprite a name." );
			}
		} else if ( displayList[i].textInstance != NULL ) {
			displayList[i].textInstance->depth = displayList[i].depth;
			if ( displayList[i].textInstance->name.c_str()[0] == '\0' ) {
				idLib::Warning( "SWF: SwapDepths called on unnamed edittext. Please give this edittext a name." );
			}
		}
	}
...
uint32 idSWFSpriteInstance::FindFrame( const char * labelName ) const {
	...
	for ( int i = 0; i < sprite->frameLabels.Num(); i++ ) {
		if ( idStr::Icmp( sprite->frameLabels[i].frameLabel.c_str(), labelName ) == 0 ) {
...
========================
*/
float idSWFSpriteInstance::GetYPos() const {
	if ( parent == NULL ) {
		return 0.0f;
	}
...
void idSWFSpriteInstance::SetXPos( float xPos ) {
	...
	swfDisplayEntry_t * thisDisplayEntry = parent->FindDisplayEntry( depth );
	if ( thisDisplayEntry == NULL || thisDisplayEntry->spriteInstance != this ) {
		idLib::Warning( "SetXPos: Couldn't find our display entry in our parents display list for depth %d", depth );
		return;
	}
...
void idSWFSpriteInstance::SetYPos( float yPos ) {
	...
	swfDisplayEntry_t * thisDisplayEntry = parent->FindDisplayEntry( depth );
	if ( thisDisplayEntry == NULL || thisDisplayEntry->spriteInstance != this ) {
		idLib::Warning( "SetYPos: Couldn't find our display entry in our parents display list for depth %d", depth );
		return;
	}
...
idSWFScriptObject_SpriteInstancePrototype::idSWFScriptObject_SpriteInstancePrototype() {
	...
	SWF_SPRITE_NATIVE_VAR_SET( _followBorder );
...
SWF_SPRITE_FUNCTION_DEFINE( duplicateMovieClip ) {
	...
	swfDisplayEntry_t * display = pThis->parent->AddDisplayEntry( 16384 + parms[1].ToInteger(), thisDisplayEntry->characterID, name );
	if ( display == NULL ) {
		return idSWFScriptVar();
	}
...
SWF_SPRITE_NATIVE_VAR_DEFINE_SET( _followBorder ) {
	SWF_SPRITE_PTHIS_SET( "_followBorder" );
	pThis->followBorder = value.ToInteger();
}

SWF_SPRITE_NATIVE_VAR_DEFINE_GET( _followBorder ) {
	SWF_SPRITE_PTHIS_GET( "_followBorder" );
	return pThis->followBorder;
}
