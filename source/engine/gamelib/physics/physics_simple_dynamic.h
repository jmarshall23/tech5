...
public:
    struct physicsSimpleDynamicData_t {
        idVec3 origin;
        idMat3 axis;
    ...
    ~idPhysics_Simple_Dynamic() override;
    ...
    bool IsInNonResidentCollisionArea(bool) override;
    bool IsOutsideWorld() override;

    physicsSimpleDynamicData_t previous;
    physicsSimpleDynamicData_t current;
...
static_assert(sizeof(idPhysics_Simple_Dynamic::physicsSimpleDynamicData_t)
        == 72, "Recovered simple-dynamic state ABI changed");
static_assert(sizeof(idPhysics_Simple_Dynamic) == 280,
    "Recovered simple-dynamic physics ABI changed");
#endif
